...
4. [Task Development Lifecycle](#task-development-lifecycle)
5. [Worklog Management](#worklog-management)
...
**Key terms used throughout this SOP:**

- **Task**: High-level work unit tracked in TODO.yaml (e.g., `T-02-02-sparse-index`)
  - Spans multiple days, sessions, and phases
  - Has dedicated git worktree and branch

- **Phase**: Implementation stage within a task (e.g., Phase 1: Planning, Phase 2: Implementation)
  - Multiple sessions can be in same phase
  - Used as checkpoint boundary

- **Day**: Calendar date in YYYY-MM-DD format (e.g., `2025-11-02`)
  - Can have multiple sessions per day
  - Used in file naming patterns

- **Session**: Focused work period within a day, typically 1-8 hours
  - Numbered sequentially across all days: 01, 02, 03, 04, 05, ...
  - File naming: `{date}-S{NN}-work-summary.md`
  - One RAW log file per session
  - **Important**: Multiple sessions can occur on same day (e.g., Sessions 01-03 all on 2025-10-31)

- **Thread**: Work unit within a session, typically 30 minutes - 2 hours
  - Numbers **reset to 01** for each new session
  - Used in RAW log organization (e.g., Session 04 has Threads 01-07, Session 05 has Threads 01-03)

- **Worktree**: Git feature allowing multiple working directories from same repository
  - Each task has dedicated worktree: `.worktrees/T-XX-XX-task-name/`
  - IDE-friendly symlink: `~/dev-space/CDSAgent-T-XX-XX-task-name`

**Session-Based vs. Daily Development**:

This SOP historically referred to "daily development," but in practice, development is **session-based**:

- **Old terminology**: "Daily worklog" (one per calendar day)
- **New terminology**: "Session worklog" (multiple possible per day)
- **File naming evolved**: `{date}-work-summary.md` → `{date}-S{NN}-work-summary.md`
...
Task: T-02-02-sparse-index
  Phase 1: Planning
    Day 2025-10-31:
      Session 01: Threads 01-03 (1.2h)
      Session 02: Threads 01-03 (1.5h) ← Same day, different session
      Session 03: Threads 01-04 (1.2h) ← Same day, third session
  Phase 2: Implementation
    Day 2025-11-01:
      Session 04: Threads 01-07 (3.2h)
    Day 2025-11-02:
      Session 05: Threads 01-03 (2.0h)
```

**See Also**: [SESSION_INITIALIZATION_WORKFLOW.md](../.dev/workflows/SESSION_INITIALIZATION_WORKFLOW.md) for session-based workflow guide
...
./.dev/scripts/task/create-task-worklog.sh T-05-01-jsonrpc-schema \
  "JSON-RPC Schema Definition & Validation" \
  "Your Name"

# Step 3: Create today's worklog
./.dev/scripts/task/create-daily-worklog.sh T-05-01-jsonrpc-schema
...
./.dev/scripts/task/worktree-symlink.sh create T-XX-XX-task-name
...
# ./.dev/scripts/task/create-task-worklog.sh ...  # Artifacts won't appear in worktree!
```
...
/Users/arthur/dev-space/CDSAgent/.dev/scripts/task/create-daily-worklog.sh T-XX-XX-task-name
...
### Phase 2: Session-Based Development

**Important Terminology**:

- **Day**: Calendar date (YYYY-MM-DD) - multiple sessions can occur per day
- **Session**: Focused work period (1-8h) - tracks one development iteration
- **Thread**: Work unit within session (30min-2h) - numbered 01, 02, 03, etc. per session

**Note**: Historically called "Daily Development", but in practice development is **session-based**. You may have multiple sessions per day (e.g., Session 01-03 all on 2025-10-31).

**Session file naming**: `{date}-S{NN}-work-summary.md` (not just `{date}-work-summary.md`)
...
```shell
# 1. Create session worklog files (NEW - Session-specific)
cd ~/dev-space/CDSAgent-T-XX-XX-task-name

# Determine next session number
ls .artifacts/spec-tasks-T-XX-XX/worklogs/ | grep -E "S[0-9]{2}" | sort | tail -1
# Example: 2025-11-01-S04-work-summary.md
# Next session: 05

# Run session initialization (recommended)
...
# Fallback: Old daily script (creates {date} files, not session-specific)
cd ~/dev-space/CDSAgent
./.dev/scripts/task/create-daily-worklog.sh T-05-01-jsonrpc-schema
...
# 4. Fill out "Session Objectives" in session-specific work-summary.md
SESSION_NUM=05  # Determined from step 1
DATE=$(date +%Y-%m-%d)
SESSION_FILE="${DATE}-S${SESSION_NUM}-work-summary.md"
vim .artifacts/spec-tasks-T-05-01-jsonrpc-schema/worklogs/${SESSION_FILE}
# Fill out:
# - Session Objectives (checkbox list)
# - Expected deliverables
```

**See also**: [SESSION_INITIALIZATION_WORKFLOW.md](../.dev/workflows/SESSION_INITIALIZATION_WORKFLOW.md) for detailed session initialization guide
...
# 2. Create RAW log (AFTER session completes)
SESSION_NUM=05
THREAD_START=01
THREAD_END=04  # Last thread number
DATE=$(date +%Y-%m-%d)

./.dev/scripts/session/create-raw-log.sh \
  T-XX-XX-task-name ${SESSION_NUM} ${THREAD_START} ${THREAD_END} "Phase 2 Testing"

# Creates: worklogs/raw/WORK-SESSIONS-05-THREADS-01-04-SUMMARY-{date}.txt
# Fill out: Thread-by-thread narrative, session summary, metrics

# 3. Fill out session work-summary.md
vim .artifacts/spec-tasks-T-XX-XX/worklogs/${DATE}-S${SESSION_NUM}-work-summary.md
# - Mark completed objectives
# - Document key decisions
# - Note blockers
# - List next session's tasks

# 4. Run checkpoint workflow
# See: .dev/workflows/WORK_SESSION_CHECKPOINT_WORKFLOW.md
# - Update metadata.yaml
# - Update TODO.yaml (optional)
# - Commit all artifacts
# - Add git notes to commits

# 5. Update task metadata
vim .artifacts/spec-tasks-T-05-01-jsonrpc-schema/metadata.yaml
# - Add session entry to 'notes:' field
# - Add session to 'worklog.entries:' with RAW log path
# - Update metrics (hours, lines of code)
# - Update acceptance criteria status
```

**See also**: [WORK_SESSION_CHECKPOINT_WORKFLOW.md](WORK_SESSION_CHECKPOINT_WORKFLOW.md) for complete end-of-session process
...
./.dev/scripts/task/create-daily-worklog.sh T-XX-XX YYYY-MM-DD

# Example for yesterday
./.dev/scripts/task/create-daily-worklog.sh T-05-01 2025-10-18
```
...
./.dev/scripts/task/create-daily-worklog.sh T-XX-XX
...
```shell
# Task management
./.dev/scripts/task/create-task-worklog.sh <ID> "<TITLE>" "<NAME>"
./.dev/scripts/task/create-daily-worklog.sh <ID> [DATE]
./.dev/scripts/task/sync-worktrees.sh
...
- **Scripts**:
  ...
  - [create-daily-worklog.sh](../.dev/scripts/task/create-daily-worklog.sh)
...
./.dev/scripts/task/create-daily-worklog.sh T-05-01
cd ~/dev-space/CDSAgent-T-05-01-jsonrpc-schema
