...
**Tasks**: CDSAgent v0.1.0-MVP Tasks DEV-COOKING Job
...
3. [Work Session Lifecycle](#work-session-lifecycle)
4. [RAW Log Management](#raw-log-management)
5. [Metadata Updates](#metadata-updates)
6. [Checkpoint Workflow](#checkpoint-workflow)
...
- **Task**: High-level work unit tracked in TODO.yaml
  - Example: `T-02-02-sparse-index`
  - Spans multiple days, sessions, and phases

- **Phase**: Implementation stage within a task
  - Example: Phase 1 (Planning), Phase 2 (Implementation)
  - Used as checkpoint boundary
  - Multiple sessions can be in same phase

- **Day**: Calendar date (YYYY-MM-DD format)
  - Example: `2025-11-02`
  - Can have multiple sessions per day
  - Used in file naming: `{date}-S{NN}-*`

- **Session**: Focused work period within a day (typically 1-8 hours)
  - Example: Session 05 (numbered sequentially across all days)
...
- **Thread**: Work unit within a session (typically 30 minutes - 2 hours)
  ...
  - Used in RAW log organization
  - Represents focused work on specific objective

**Important Distinctions**:

| Concept | Scope | Reset Per | File Naming Example |
|---------|-------|-----------|---------------------|
| Task | Entire feature/issue | Never | T-02-02-sparse-index |
| Phase | Implementation stage | Per phase | Phase 2 |
| Day | Calendar date | Daily | 2025-11-02 |
| Session | Work period | Never (sequential: 01, 02, 03...) | 2025-11-02-S05-* |
| Thread | Work unit | Per session (resets to 01) | Thread 01, 02, 03 |
...
Task: T-02-02-sparse-index
  Phase 1: Planning
    Day 2025-10-31:
      ...
      Session 02: Threads 01-03 ← Thread numbers reset
      Session 03: Threads 01-04 ← Thread numbers reset again
  Phase 2: Implementation
    Day 2025-11-01:
      Session 04: Threads 01-07 ← Thread numbers reset
    Day 2025-11-02:
      Session 05: Threads 01-03 ← Thread numbers reset
```
...
1. **Initialize Session Files** (2 minutes with script, 10 minutes manual):
   ...
   **Automated (Recommended)**:
   ...
   ./.dev/scripts/session/create-session-worklog.sh \
     T-XX-XX-task-name 05 "Session Description" "Your Name"
   ...
   **Manual (Fallback)**:
   ...
   ```shell
   # Copy templates and replace placeholders manually
   cp .dev/templates/worklogs/work-summary.template.md \
      .artifacts/spec-tasks-T-XX-XX/worklogs/{date}-S05-work-summary.md
   # Repeat for commit-log, notes, codereview
   # Edit each file: replace {DATE}, {SESSION}, {TASK_ID}, etc.
   ```

2. **Fill Out Session Objectives** in `{date}-S{NN}-work-summary.md`:
...
3. **Begin Development** (Thread 01):
   - Start coding, researching, planning
   - Make commits with conventional commit messages
   - Update session files continuously

4. **DO NOT** create RAW log yet (will create when session completes)

**Artifacts Created**:

- `{date}-S{NN}-raw-log.md` ← RAW log file
- `{date}-S{NN}-work-summary.md` ← Session-specific file
- `{date}-S{NN}-commit-log.md`
- `{date}-S{NN}-notes.md`
- `{date}-S{NN}-codereview.md` (optional, typically created during testing phase)

**See Also**: [SESSION_INITIALIZATION_WORKFLOW.md](../.dev/workflows/SESSION_INITIALIZATION_WORKFLOW.md) for detailed guide
...
Based on `.dev/workflows/WORK_SESSION_CHECKPOINT_WORKFLOW.md`:
