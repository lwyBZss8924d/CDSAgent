...
5. [Phase-by-Phase Guides](checkpoint/)
...
6. [Reference Materials](checkpoint/)
...
- **WORKTREE_WORKFLOW.md**: Defines overall task development lifecycle
  - Phase 2 (Session-Based Development) → leads to → **This Checkpoint Workflow** (End of Session)
- **NEXT_TASK_CHECKLIST.md**: Defines task initialization
  - Task starts → Initialize artifacts → Session dev → **Checkpoint** → Complete task

---

## Terminology Glossary

**Key terms used in checkpoint workflow:**

- **Session**: Focused work period (1-8h) that is the unit of checkpoint
  - Example: Session 04 (3.2h, 7 threads)
  - One checkpoint per session
  - Numbered sequentially: 01, 02, 03, 04, 05, ...

- **Thread**: Work unit within session (30min-2h)
  - Example: Thread 01, 02, 03 within Session 04
  - Numbers **reset to 01** for each new session
  - Used in RAW log organization

- **RAW Log**: Complete session narrative for AI handoff
  - File: `WORK-SESSIONS-{NN}-THREADS-{START}-{END}-SUMMARY-{DATE}.txt`
  - Location: `.artifacts/spec-tasks-T-XX-XX/worklogs/raw/`
  - Created **AFTER session completes** (not before!)

- **Artifacts**: Session documentation files
  - `{date}-S{NN}-work-summary.md` - Objectives, completed work, decisions
  - `{date}-S{NN}-commit-log.md` - Git commit tracking
  - `{date}-S{NN}-notes.md` - Technical notes, research
  - `{date}-S{NN}-codereview.md` - Testing results, fixes (optional)

- **Git Notes**: Metadata attached to commits
  - Format: `spec-tasks/T-XX-XX\nDay: X\nDate: YYYY-MM-DD\n...`
  - Pushed separately: `git push origin refs/notes/commits`
  - Required for checkpoint completion

- **Checkpoint**: End-of-session artifact consistency verification
  - Ensures git reality matches documentation
  - 5 phases: Review → Verification → Update → Git Operations → Final Verification
  - Result: 100% consistency between code, commits, and docs

**Checkpoint vs. Daily Development**:

- **Old terminology**: "End-of-day" (EOD) checkpoint
- **New terminology**: "End-of-session" checkpoint
- **Reality**: Sessions don't align with calendar days (can have 3 sessions in one day)

**Example Checkpoint Frequency**:

```text
Day 2025-10-31:
  Session 01 (1.2h) → Checkpoint 01
  Session 02 (1.5h) → Checkpoint 02
  Session 03 (1.2h) → Checkpoint 03
Day 2025-11-01:
  Session 04 (3.2h) → Checkpoint 04
```

**See Also**: [WORKLOG-HANDBOOK.md](../.artifacts/WORKLOG-HANDBOOK.md) for complete session lifecycle
...
All checkpoint documentation is located in `.dev/workflows/checkpoint/`:
...
- **v1.1** (2025-10-27): Split into focused chapters in .dev/workflows/checkpoint/ directory for better IDE performance and navigation
